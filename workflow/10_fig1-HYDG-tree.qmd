...
create_tree <- function(ftree, df_anno_imgvr, df_anno_nr) {
  ...
  tips_to_remove <- sapply(subs_to_collapse, function(x) return(x$tip.label))
  
  # remove the previous subtrees from the original tree
  pruned_phylo <- tree_phylo
  
  for (i in seq(1,length(tips_to_remove))) {
    if (all(tips_to_remove[[i]] %in% pruned_phylo$tip.label)) {
      pruned_phylo <- ape::drop.tip(pruned_phylo, tips_to_remove[[i]], trim.internal = TRUE)
      # print(pruned_phylo$Nnode)
      # print(length(pruned_phylo$tip.label))
    }
  }
  ...
  my_colors <- c(colorRampPalette(my_colors)(length(unique(pruned_data$Phylum))-1),"#787777")
  my_colors2 <- c("#5f0f40","#9a031e","#fb8b24","#BC5210","#0f4c5c")
  my_colors2 <- c(colorRampPalette(my_colors2)(length(unique(pruned_data$eco3))-1),"#787777")
  ...
  p <- ggtree(pruned_phylo, layout = "circular", color="black", size=0.2) 

  p <- p %<+% pruned_data +
    geom_tippoint(aes(fill=Phylum), size=0.7, alpha=0.7, shape=21, color="transparent",
                  show.legend = FALSE) +
    
    geom_tiplab(color="black", size=1, align=FALSE) +
    
    # FOR SOME REASON TIP LABELS GIVE AN ERROR WHEN ALIGN IS SET TO TRUE ?????
    # geom_tiplab(color="black", size=0.8, align=TRUE,
    #             linetype = "dotted", linesize = 0.2) +
